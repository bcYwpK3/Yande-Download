...
class Application:
    ...
    def get_page(self, page, pool):
        ...
        for pic in pic_list[:]:
            ...
            filepath = self._path(pic)
            if exists(filepath):
                if self._get_size(filepath) == pic['file_size']:
                    pic_list.remove(pic)
        '''别移没了'''
        if pic_list_len <= 0:
            ui.no('已下载完毕 跳过')
    ...
    def _get_pic_list(self, page):
        '''请求图片列表API'''
        url = f'https://yande.re/post.json?limit=100&page={page}&tags={self.tags}{self.except_tags}'
        '''调试参数，将url的值输出到当前目录下url.txt文件内'''
    ...
    def _path(self, pic):
        '''根据图片元数据和保存位置生成对应路径'''
        # filepath = join(self.save_dir, '%d.%s' % (pic['id'], pic['file_ext']))
        data = pic['sample_url']
        original = os.path.basename(urlparse(data).path)
        unoriginal = original[:-4]
        '''将Unicode进行转换'''
        filename = urllib.parse.unquote(unoriginal)
        '''处理windows下非法字符'''
        pattern = r'[\\/:*?"<>| ]'
        patternfile = re.sub(pattern, '_', filename)
        if patternfile:
            filepath = os.path.join(self.save_dir, '%s.%s' % (patternfile, pic['file_ext']))
        '''
        调试参数
        将data的值和filepath的值进行输出
        到当前目录下url.txt文件内
        '''
        with open('post.txt', 'w') as file:
            file.write(data + '\n')
            file.write(original + '\n')
            file.write(unoriginal + '\n')
            file.write(patternfile + '\n')
            file.write(filename + '\n')
            file.write(filepath + '\n')
            file.write('\n')
        return filepath
